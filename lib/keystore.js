...
var KeyStore = function (webinosType, webinosRoot) {
    ...
    var KeyStoreContext = this, userData, keyStorage; // OwnInstance
    ...
    function initializeKeyStorage() {
        ...
        keyStorage.put = function(id, value) {
            fs.writeFileSync(path.resolve(path.join(webinosRoot, "keys", id)), value);
        };
        keyStorage.get = function(id) {
            var data = fs.readFileSync(path.resolve(path.join(webinosRoot, "keys", id)));
            return data.toString();
        };
        keyStorage.delete = function(id) {
            fs.unlinkSync(path.resolve(path.join(webinosRoot, "keys", id)));
            return true;
        };
